...
class LlmProvider:
    ...
    def get_llm(self):
        # The response array can be customized via the 'vendorConfig'
        response_array = self.config.get("responseArray", [
            "This is a default response."
        ])

        # Format any placeholders in the response array
        formatted_responses = [resp.format(self_user_id=self.user_id) for resp in response_array]
    ...
    def get_system_prompt(self):
        ...
        return system_prompt.format(self_user_id=self.user_id)
