...
class SessionManager:
    """
    Manages the core connectivity session for a bot.
    Responsibilities:
    1.  Initialize and manage the Chat Provider (WhatsApp connection).
    2.  Initialize and manage the Bot Queues.
    ...
    def _initialize_components(self):
        ...
        logging.info(f"SESSION ({self.bot_id}): Initializing core components...")
        ...
        try:
            ...
            logging.info(f"SESSION ({self.bot_id}): Initialized chat provider '{provider_name}'.")
            
        except Exception as e:
            logging.error(f"SESSION ({self.bot_id}): Failed to initialize provider: {e}")
            raise RuntimeError(f"Failed to initialize provider for {self.bot_id}: {e}")
    ...
    def register_message_handler(self, handler: Callable[[str, Message], Awaitable[None]]):
        ...
        logging.info(f"SESSION ({self.bot_id}): Registered message handler: {handler.__name__ if hasattr(handler, '__name__') else str(handler)}")
    ...
    def register_service(self, service: Any):
        ...
        logging.info(f"SESSION ({self.bot_id}): Registered associated service: {service.__class__.__name__}")
    ...
    def register_feature(self, name: str, service: Any):
        ...
        logging.info(f"SESSION ({self.bot_id}): Registered feature '{name}': {service.__class__.__name__}")
    ...
    async def _on_queue_message(self, user_id: str, correspondent_id: str, message: Message):
        ...
        for handler in self._message_handlers:
            try:
                ...
                logging.error(f"SESSION ({self.bot_id}): Error in feature handler: {e}")

    async def start(self):
        """Starts the provider listener."""
        if not self.provider_instance:
            raise RuntimeError(f"Session {self.bot_id} cannot start: provider not initialized.")

        logging.info(f"SESSION ({self.bot_id}): Starting provider listener...")
        await self.provider_instance.start_listening()
        logging.info(f"SESSION ({self.bot_id}): Session started.")

    async def stop(self, cleanup_session: bool = False):
        """Stops the provider listener and associated services."""
        logging.info(f"SESSION ({self.bot_id}): Stopping session... (cleanup={cleanup_session})")
        
        # Stop associated services (LIFO order typical for shutdown)
        for service in reversed(self._associated_services):
            if hasattr(service, 'stop'):
                try:
                    logging.info(f"SESSION ({self.bot_id}): Stopping service {service.__class__.__name__}...")
                    if inspect.iscoroutinefunction(service.stop):
                        await service.stop()
                    else:
                        service.stop()
                except Exception as e:
                    logging.error(f"SESSION ({self.bot_id}): Error stopping service {service.__class__.__name__}: {e}")

        if self.provider_instance:
            await self.provider_instance.stop_listening(cleanup_session=cleanup_session)
            
        logging.info(f"SESSION ({self.bot_id}): Session stopped.")
