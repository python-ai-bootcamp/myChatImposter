...
class TokenTrackingCallback(AsyncCallbackHandler):
    ...
    async def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        try:
            ...
            # --- Strategy 2: Provider-Specific Normalizer (Fallback) ---
            if input_tokens == 0 and output_tokens == 0:
                # If standard metadata failed or yielded 0, try llm_output
                input_tokens, output_tokens = self._extract_provider_specific_usage(response.llm_output)

            # --- Record Event (only if usage detected or force 0 if that's the reality) ---
            # NOTE: We allow recording 0 if the provider explicitly returned 0, 
            # but usually providers return valid numbers. 
            # If both are 0, it might mean extraction failed, but we log a warning in that case.
            ...
            if input_tokens > 0 or output_tokens > 0:
                await self.token_service.record_event(
                 ...
                 # If we have output but extraction failed to find tokens
                 logger.warning(
            ...
            # Swallow exceptions to not crash the main flow, but log error
            logger.error(f"TokenTrackingCallback error: {e}")

    def _extract_provider_specific_usage(self, llm_output: Optional[Dict]) -> tuple[int, int]:
        """
        ...
        # OpenAI style: 'token_usage': {'prompt_tokens': 1, 'completion_tokens': 1, 'total_tokens': 2}
        if 'token_usage' in llm_output:
            usage = llm_output['token_usage']
            input_t = usage.get('prompt_tokens', 0)
            output_t = usage.get('completion_tokens', 0)
            
        # Add other providers here as needed (e.g. Anthropic 'usage')
