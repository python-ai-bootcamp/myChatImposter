...
The only UI-facing changes are:
...
2. **Frontend Validation Definition (`configModels.js`)**: The frontend `LLMConfigurations.validate()` method currently routes all three tiers (`high`, `low`, `image_moderation`) to the `LLMProviderConfig` validation class, which enforces chat-specific fields like `temperature` and `record_llm_interactions`. This validation function must be **overhauled** to explicitly route the `image_moderation` property to the newly added `BaseModelProviderConfig` validation loop. The new class must securely validate only `model`, `api_key_source`, and `api_key` to avoid a `ValidationError` when the stripped backend database configurations reach the UI.
...
| `utils/provider_utils.py` | `find_provider_class` — no `inspect.isabstract()` check | **UPDATE** — add `inspect.isabstract(obj)` filter to skip abstract intermediate classes. **Recommendation**: Apply this fix **first** before creating new provider classes to prevent `find_provider_class` from attempting to instantiate abstract classes at runtime. |
...
| `frontend/src/configModels.js` | `this.image_moderation = new LLMProviderConfig(image_moderation || {})` | **ADD** a dedicated `BaseModelProviderConfig` JS class expecting only `BaseModelProviderSettings` fields (`api_key_source`, `api_key`, `model`, `provider_name`); update `LLMConfigurations` constructor to use it for `image_moderation` while keeping `LLMProviderConfig` for `high`/`low`. Keeps the frontend data model contract aligned with the new backend type hierarchy. **CRITICAL**: explicitly update `LLMConfigurations.validate()` to route the `image_moderation` property to the new `BaseModelProviderConfig.validate()`, and ensure the new class only checks `model`, `api_key_source`, and `api_key` without enforcing chat-specific fields like `temperature` or `record_llm_interactions`. |

> **Warning — `fakeLlm.py` Test-Blocker**: Beyond the import change, `FakeLlmProvider` in `model_providers/fakeLlm.py` currently uses `self.user_id` (line 88: `resp.format(user_id=self.user_id)`). Since `user_id` is dropped from the provider hierarchy (Section 3.1), this reference will crash at runtime. Furthermore, **the file is missing `from typing import Optional, List, Any`**. This is a critical test-blocker: modern test frameworks introspect type hints, and this missing import will cause immediate `NameError` crashes during the refactor sweep. Both the `user_id` dependency and the missing imports must be fixed concurrently. Finally, a full sweep of unit and integration tests (especially `tests/integration/test_token_flow_component.py`) must be conducted. Any test asserting against the exact output block of `fakeLlm.py` (which contained the `user_id`) must be updated to expect the new parameterless response string.
